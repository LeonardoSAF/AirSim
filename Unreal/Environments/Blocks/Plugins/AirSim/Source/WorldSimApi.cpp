...
void WorldSimApi::printLogMessage(const std::string& message,
    const std::string message_param, unsigned char severity, float persist_sec)
{
    UAirBlueprintLib::LogMessageString(message, message_param, static_cast<LogDebugLevel>(severity), persist_sec);
}

const std::set<std::string>& WorldSimApi::listSceneAllObjects() const
{
    static std::set<std::string> objectCache;
    UAirBlueprintLib::RunCommandOnGameThread([this]() {
        UAirBlueprintLib::ListActors(simmode_, objectCache);
    }, true);
    return objectCache;
}
...
class MultirotorCamera : public msr::airlib::CameraBase {
    APIPCamera* camPip;
    std::string name;
...
public:
    MultirotorCamera(APIPCamera* camPipIn, const std::string& nameIn) : camPip(camPipIn), name(nameIn){
        engineName = std::string(TCHAR_TO_UTF8(*camPip->GetName()));
        game_viewport_ = camPip->GetWorld()->GetGameViewport();
        capture = camPip->getCaptureComponent(APIPCamera::ImageType::Scene, false);
    }
    ...
    virtual std::unique_ptr<SceneBase> render() override {
        std::unique_ptr<SceneBase> scene = std::make_unique<SceneBase>(this);
        return scene;
    }
};

class Multirotor : public msr::airlib::MultirotorBase {
    MultirotorPawnSimApi* multi;
    msr::airlib::MultirotorApiBase* rotor;
    float timeout_sec = 2;
    std::shared_ptr<std::vector<std::shared_ptr<CameraBase>>> _cameraList = nullptr;
public:
    Multirotor(MultirotorPawnSimApi* multiIn, msr::airlib::MultirotorApiBase* rotorIn) : multi(multiIn), rotor(rotorIn) {
        _cameraList = std::make_shared<std::vector<std::shared_ptr<CameraBase>>>();
        for (const std::pair<std::string, APIPCamera*> camPip : multi->getCameras().getMap()) {
            std::shared_ptr<MultirotorCamera> cam = std::make_shared<MultirotorCamera>(camPip.second, camPip.first);
    ...
    virtual std::shared_ptr<std::vector<std::shared_ptr<CameraBase>>> listCameras() override {
        return _cameraList;
    }

    virtual std::shared_ptr<std::vector<uint8>> getImage() override {
        return std::make_shared<std::vector<uint8>>(this->multi->getImage(multi->getFirstCameraName(), msr::airlib::ImageCaptureBase::ImageType::Scene));
    }

    virtual std::shared_ptr<msr::airlib::Image> getImageWithMeta(const std::shared_ptr<CameraBase>& camera) override {
        auto imageResp = this->multi->getImageAsResponse(camera->getName(), msr::airlib::ImageCaptureBase::ImageType::Scene);
        return std::make_shared<msr::airlib::Image>(camera->getName(), imageResp.image_data_uint8, imageResp.width, imageResp.height);
    }

    virtual std::shared_ptr<std::vector<std::shared_ptr<Image>>> getAllImagesWithMeta() override {
        return getImagesWithMeta(*_cameraList);
    }

    virtual std::shared_ptr<std::vector<std::shared_ptr<Image>>> getImagesWithMeta(const std::vector<std::shared_ptr<CameraBase>>& cameras) override {
        std::vector<msr::airlib::ImageCaptureBase::ImageRequest> requests;
        for (auto& cam : cameras) {
            requests.emplace_back(cam->getName(), ImageCaptureBase::ImageType::Scene);
        }
        auto imageResps = this->multi->getImages(requests);
        std::shared_ptr<std::vector<std::shared_ptr<msr::airlib::Image>>> images = std::make_shared<std::vector<std::shared_ptr<msr::airlib::Image>>>();
        for (auto& imageResp : imageResps) {
            images->push_back(std::make_shared<msr::airlib::Image>(imageResp.camera_name, imageResp.image_data_uint8, imageResp.width, imageResp.height));
        }
        return images;
    }

    virtual void rotateAntiClockwise(float rate, float amount_of_time_secs) override {
        rotor->rotateByYawRate(-rate, amount_of_time_secs);
    }

    virtual void rotateClockwise(float rate, float amount_of_time_secs) override {
        rotor->rotateByYawRate(rate, amount_of_time_secs);
    }
        
    virtual void requestApiControl() override {
        rotor->enableApiControl(true);
    }

    virtual void takeoff() override {
        if (!rotor->isApiControlEnabled()) {
            rotor->enableApiControl(true);
        }
        if (rotor->canArm()) {
            rotor->armDisarm(true);
        }
        rotor->takeoff(2);
    }
};

std::shared_ptr<msr::airlib::MultirotorBase> WorldSimApi::getMultirotorVehicleApi(const std::string& vehicleName) const {
    MultirotorPawnSimApi* multi = static_cast<MultirotorPawnSimApi*>(simmode_->getVehicleSimApi(vehicleName));
    msr::airlib::MultirotorApiBase* rotor = multi->getVehicleApi();
    return std::make_shared<Multirotor>(multi, rotor);
}