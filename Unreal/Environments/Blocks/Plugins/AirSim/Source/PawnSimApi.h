...
class PawnSimApi : public msr::airlib::VehicleSimApiBase {
    std::shared_ptr<std::vector<APIPCamera*>> _cameraList = nullptr;
public: //types
...
public: //implementation of VehicleSimApiBase
    ...
    virtual ImageCaptureBase::ImageResponse getImageAsResponse(const std::string& camera_name, ImageCaptureBase::ImageType image_type) const override;
...
public: //Unreal specific methods
    ...
    APIPCamera* getFirstCamera() {
        return *cameras_.begin();
    }

    std::string getFirstCameraName() {
        return cameras_.getMap().begin()->first;
    }

    std::string getAnyCameraName() {
        return (cameras_.getMap().begin()++)->first;
    }
