...
namespace lca {
    namespace container {
        __declspec(dllimport) int runFramework(int argc, char* argv[], std::atomic<bool>& shutdown, msr::airlib::WorldSimApiBase* base);
    }
}
...
void ASimModeWorldMultiRotor::BeginPlay()
{
    ...
    initializeForPlay();
    this->lca_thread = std::make_unique<std::thread>([this]() {
        int argc = 1;
        char* argv[] = { "" };
        int res = lca::container::runFramework(argc, argv, this->shutdown, this->getApiProvider()->getWorldSimApi());
    });
}
