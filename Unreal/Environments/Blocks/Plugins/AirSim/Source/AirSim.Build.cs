...
public class AirSim : ModuleRules
{
    ...
    {
        System.Console.WriteLine("Building LCA AIRSIM...");
        System.Console.WriteLine("  Current Working Directory: " + Directory.GetCurrentDirectory());
        System.Console.WriteLine("           Module Directory: " + ModulePath);
        string lcaPath = Path.Combine(ModulePath, "..", "..", "..", "..", "..", "..", "leonardo_synthetic_env");
        System.Console.WriteLine("              LCA Directory: " + lcaPath + " " + Directory.Exists(lcaPath));
        PublicDelayLoadDLLs.Add("RefContainer_Container.dll");
        PublicAdditionalLibraries.Add(Path.Combine(lcaPath, "out", "build", "x64-Debug", "RefContainer_ContainerBuild", "RefContainer_Container.lib"));
        PublicIncludePaths.Add(Path.Combine(lcaPath, "LCA", "ContainerFramework"));
        string[] files = Directory.GetFiles(Path.Combine(lcaPath, "out", "build", "x64-Debug"), "*", SearchOption.TopDirectoryOnly);
        foreach (string file in files){
            CopyFileIfNewer(file, @"C:\Users\Admin\AirSimLCA\Unreal\Environments\Blocks\Binaries\Win64");
        }
    ...
    static void CopyFileIfNewer(string srcFilePath, string destFolder)
    {
        ...
        {
            System.Console.WriteLine("Copying " + srcFile + " to " + destFile.FullName + "...");
