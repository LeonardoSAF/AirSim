...
namespace msr { namespace airlib {
   ...
   class SceneBase;
    ...
    public:
        ...
        virtual std::unique_ptr<SceneBase> render() = 0;
    };
...
protected:
    CameraBase* owner;
public:
    SceneBase(CameraBase* ownerIn) : owner(ownerIn){}
};
...
public:
    ...
    Image(std::string ownerIn, std::vector<std::uint8_t> dataIn, std::uint32_t widthIn, std::uint32_t heightIn) {
        owner = ownerIn;
        data = dataIn;
        width = widthIn;
        height = heightIn;
...
public:
    virtual ~MultirotorBase() = default;
    virtual void rotateAntiClockwise(float rate, float amount) = 0;
    virtual void rotateClockwise(float rate, float amount) = 0;
    virtual std::shared_ptr<std::vector<std::uint8_t>> getImage() = 0;
    virtual std::shared_ptr<std::vector<std::shared_ptr<Image>>> getImagesWithMeta(const std::vector<std::shared_ptr<CameraBase>>& cameras) = 0;
    virtual std::shared_ptr<std::vector<std::shared_ptr<Image>>> getAllImagesWithMeta() = 0;
    virtual std::shared_ptr<msr::airlib::Image> getImageWithMeta(const std::shared_ptr<CameraBase>& camera) = 0;
    virtual std::shared_ptr<std::vector<std::shared_ptr<CameraBase>>> listCameras() = 0;
    virtual void requestApiControl() = 0;
...
public:
    ...
    virtual void setTimeOfDay(bool is_enabled, const std::string& start_datetime, bool is_start_datetime_dst, float celestial_clock_speed, float update_interval_secs, bool move_sun) = 0;
    ...
    virtual void printLogMessage(const std::string& message, const std::string message_param = "", unsigned char severity = 0, float persist_sec = 2) = 0;
    ...
    virtual const std::set<std::string>& listSceneAllObjects() const = 0;
    ...
    virtual std::shared_ptr<MultirotorBase> getMultirotorVehicleApi(const std::string& vehicleName) const = 0;
};
